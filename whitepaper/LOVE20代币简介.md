...
- **核心机制：** 以 1:1 比例封装 TKM 代币为 TKM20 代币，并使用封装后的 TKM20 代币申购 LOVE20 代币。
...
- 质押流动性后，还可单独质押 LOVE20 代币获取 ST 质押凭证，持有 ST 代币可加速获取治理激励，最高可额外获得 2 倍激励。
...
- 用户可随时申请释放质押资产，申请后将立即失去治理资格并收回 SL 和 ST 代币，进入等待状态，释放期结束后可取回对应资产；
...
行动激励的获取需满足两个条件：首先，行动获得的验证票数不低于当轮投票阶段总投票数的 5%；其次，参与地址被抽中验证并获得验证票数。激励按行动和轮次分别计算，不可重复铸造。计算公式：

地址可铸造的行动激励 = 本轮行动总激励 × (该地址本轮行动获得的验证票数 / 本轮有激励行动的验证总票数)
