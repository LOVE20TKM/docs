...
2. 保底机制：发射结束前可撤回申购的 TKM20，发射结束后，募集的 TKM20 全部作为拖底池，可随时销毁代币换回同比例拖底池中的 TKM20。
...
**2. 发射结束后的取回机制：**  
用户可通过销毁 LOVE20 代币从拖底池取回相应比例的 TKM20 代币。
...
**2. 拖底池机制：**  
达到募集目标的 TKM20 将转入 LOVE20 代币合约，形成 **"拖底池"**。用户可通过 LOVE20 代币合约查询 TKM20 余额，该数值即为拖底池中的 TKM20 数量。

每位 LOVE20 代币持有者可通过销毁代币从拖底池中提取相应比例的 TKM20。例如，若拖底池中有 800 枚 TKM20，而当前已铸造 100 个 LOVE20 代币，持有其中 10%（10 个）的用户可销毁这些代币，从拖底池中提取 10% 的 TKM20（即 80 枚）。

这是从拖底池提取 TKM20 代币的唯一方式。
...
- 流动性质押产生的所有交易手续费中，TKM20 部分将注入拖底池，LOVE20 部分直接销毁。
