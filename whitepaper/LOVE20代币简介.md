...
2. 托底机制：发射结束前可撤回申购的 TKM20，发射结束后，募集的 TKM20 全部作为托底池，可随时销毁代币换回同比例托底池中的 TKM20。
...
- **TKM20 代币募集目标：** 800 枚（注：此为内测阶段数值）。

### 2.3. 代币分配机制

**按申购比例分配：** 例如，若用户申购了 100 枚 TKM20，而总申购量为 1000 枚 TKM20，则该用户将获得 10% 的发射代币，即 1 亿枚 LOVE20。
...
- **2. 时间条件：** 自 TKM20 筹集量达到目标的 50% 开始计算，最后一笔申购需至少经过 1408 个区块时间（注：此为内测阶段数值）。
...
**1. 发射结束前的取回机制：**  
申购后等待 40 个区块时间（注：此为内测阶段数值），若公平发射尚未结束，用户可随时取回全部申购的 TKM20。

**2. 发射结束后的取回机制：**  
用户可通过销毁 LOVE20 代币从托底池取回相应比例的 TKM20 代币。
...
**1. 超额募集资金的处理：**  
超过募集目标的 TKM20 将按比例退回给参与地址。例如，用户申购了 100 枚 TKM20，占总申购量 1000 枚的 10%，而募集目标为 800 枚，则超额部分 200 枚中的 10%（20 枚 TKM20）将退还给该用户。

**2. 托底池机制：**  
达到募集目标的 TKM20 将转入 LOVE20 代币合约，形成 **"托底池"**。用户可通过 LOVE20 代币合约查询 TKM20 余额，该数值即为托底池中的 TKM20 数量。

每位 LOVE20 代币持有者可通过销毁代币从托底池中提取相应比例的 TKM20。例如，若托底池中有 800 枚 TKM20，而当前已铸造 100 个 LOVE20 代币，持有其中 10%（10 个）的用户可销毁这些代币，从托底池中提取 10% 的 TKM20（即 80 枚）。

这是从托底池提取 TKM20 代币的唯一方式。
...
- 流动性质押产生的所有交易手续费中，TKM20 部分将注入托底池，LOVE20 部分直接销毁。
