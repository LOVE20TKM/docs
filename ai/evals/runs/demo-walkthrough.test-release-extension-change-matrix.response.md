...
2. `interface-test`
   如果 ABI 变了，先跑 `yarn generate:abi`；然后执行 `yarn test` 和对应 smoke path。
...
2. 如果工厂地址或扩展类型有变，更同步 `interface-test/.env*` 与 `interface-test/src/config/extensionConfig.ts`
...
3. 仅在上述检查通过且明确要求正式发布时，运行 `yarn release:test-to-interface-main`；不直接修改 `interface`
