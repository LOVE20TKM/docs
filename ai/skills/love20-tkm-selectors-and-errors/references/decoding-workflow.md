...
2. Confirm the candidate contract in `interface-test/docs/function-selectors.json`.
...
4. Open the matching ABI module in `interface-test/src/abis`.
5. Confirm the matching Solidity interface or implementation in the behavior-owning contract repo. For group-chat, verify that the target address belongs to the latest pilot deployment.
...
Choose `interface-test` for active/public-test behavior or `interface` for an already released production build, then:

1. Open `interface-test/src/errors/contractErrorParser.ts`.
2. Open `interface-test/src/errors/unifiedErrorMap.ts`.
3. Open `interface-test/src/errors/errorMessages.ts` if you need the maintained source for the Chinese translation text.
