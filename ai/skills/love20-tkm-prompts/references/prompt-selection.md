...
- Extension or derivative protocol implementation:
  use `$love20-tkm-extension-dev`; add `$love20-tkm-extension-patterns` when base-class choice or reward model is still unclear.
- Cross-repo integration or end-to-end wiring:
  use `$love20-tkm-integration-dev`; add the most relevant implementation skill when the owning repo still needs code changes.
- Contract interaction or user flow:
  use `$love20-tkm-contract-playbooks`, and add `$love20-tkm-extension-patterns` if extensions are involved.
- State query, read path, viewer, hook, or event timeline:
  use `$love20-tkm-state-and-events`.
- Failure triage:
  use `$love20-tkm-runbooks`; add `$love20-tkm-selectors-and-errors` when the symptom is a selector, topic, or raw revert.
- Frontend source mapping:
  use `$love20-tkm-frontend-bridge`; add `$love20-tkm-state-and-events` if the ask includes viewers or event history.
- Frontend feature or extension UI implementation:
  use `$love20-tkm-frontend-dev`; add `$love20-tkm-frontend-bridge` when you first need to trace the nearest existing page or hook chain.
- Test, regression, or release planning:
  use `$love20-tkm-test-and-release`; add `$love20-tkm-integration-dev` when the change crosses contract, script, and frontend layers.
- Acceptance or regression checks for another agent:
  use `$love20-tkm-prompts` and `references/junior-agent-acceptance-cases.md`.
